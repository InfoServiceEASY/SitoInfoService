...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $fk_utente = GetUser()[0];
  $error = InsertReport($durata, $descrizione, $isrisolto, $id, $fk_utente);
...
function InsertReport($durata, $descrizione, $isrisolto, $fk_ticket, $fk_dipendente)
{
  ...
  $conn = DataConnect();
  $sql = "update report set datafine=?,durata=?,attività=?,isrisolto=? where fk_ticket=? and fk_dipendente=? and isnull(isconvalidato)";
  //mettere ora attuale
