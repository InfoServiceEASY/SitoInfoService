...
"SELECT monthname(dataapertura) as mese,  count(*) AS count FROM ticket WHERE YEAR(dataapertura)=? GROUP BY mese, month(dataapertura) order by month(dataapertura)"
:
"SELECT monthname(dataapertura) as mese,  count(*) AS count FROM ticket inner join report on report.fk_ticket=ticket.id WHERE YEAR(ticket.dataapertura)=? and report.fk_dipendente=? GROUP BY mese, month(ticket.dataapertura) order by month(ticket.dataapertura)");
if ($_SESSION['member'] == "dipendente"){
    $stmt -> bind_param('ii', $data, GetUser()[0]);
}
else {
