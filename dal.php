...
function RitornaPercentuale_helpdesk($chiave, $total_pages, $anno)
{
    if ($total_pages > 0) {
        $conn = DataConnect();
        ...
        $stmt->close();
        $variabile= ($numero / $total_pages * 100 . "%");
return $variabile;
    } else
        return "0%";
}
function RitornaPercentuale_dipendente($chiave, $anno)
{
    $variabile=GetUser()[0];
    $conn = DataConnect();
    $sqlstmt = $conn -> prepare("SELECT count(distinct t.id) as count from ticket t inner join report r on t.id = r.fk_ticket WHERE YEAR(t.dataapertura)=? and r.fk_dipendente= ?");
    $sqlstmt -> bind_param('ii', $anno, $variabile);
    $sqlstmt -> execute();
    $result1 = $sqlstmt -> get_result();
    $var = $result1 ->fetch_assoc();
    $total_pages = intval($var['count']);
    $sqlstmt -> close(); 
    if ($total_pages > 0) {
        $conn = DataConnect();
        $stmt = $conn->prepare("SELECT count(distinct t.id) as count from ticket t inner join report r on t.id = r.fk_ticket where t.tipologia=? and YEAR(t.dataapertura)=? and r.fk_dipendente= ?");
...
function RitornaNumero($chiave, $anno, $kind_of_user){
    return intval($kind_of_user == "dipendente"? RitornaNumero_dipendente($chiave, $anno) : RitornaNumero_helpdesk($chiave, $anno));
}
function RitornaNumero_helpdesk($chiave, $anno)
{
    $conn = DataConnect();
...
function RitornaNumero_dipendente($chiave, $anno)
{
    $conn = DataConnect();
    $query = null;
    switch($chiave){
